...
const productionEntrySchema = z.object({
  ...
  phoneNumber: z.string().regex(/^[0-9]*$/, "Numbers only").length(10, "Must be 10 digits"),
...
export function NewProductionForm({ machines, karigars, onAddEntry }: NewProductionFormProps) {
  ...
  const nextStep = async () => {
    const stepFields = [
      ['date', 'shift'], 
      ['machineId', 'karigarName', 'phoneNumber'], 
                  ...
                  {currentStep === 2 && (
                    <div className="space-y-8">
                      ...
                      <div className="grid grid-cols-2 gap-4 pt-4">
                        <div className="p-5 rounded-3xl bg-slate-900 text-white shadow-xl relative overflow-hidden group">
                          <Ruler className="absolute -right-2 -bottom-2 h-16 w-16 text-white/10 -rotate-12" />
                          <p className="text-[10px] font-bold uppercase text-slate-400 mb-1">Total Meters</p>
                          <p className="text-3xl font-bold tracking-tight">{watchedValues.totalMeter || 0}m</p>
                        </div>
                        <div className="p-5 rounded-3xl bg-orange-500 text-white shadow-xl relative overflow-hidden group">
                          <Target className="absolute -right-2 -bottom-2 h-16 w-16 text-white/10 -rotate-12" />
                          <p className="text-[10px] font-bold uppercase text-orange-200 mb-1">Total Tich</p>
                          <p className="text-3xl font-bold tracking-tight">{watchedValues.totalTich || 0}</p>
                        </div>
                      </div>
                    </div>
                  )}
              ...
              <div className="flex gap-4 pt-4">
                ...
                {currentStep < 2 ? (
                  ...
                  <Button type="submit" className="flex-1 h-16 rounded-3xl font-bold text-lg bg-slate-900 hover:bg-black text-white shadow-2xl transition-all active:scale-95">
                    <Zap className="mr-2 fill-current w-5 h-5" /> Save Production
                  </Button>
                )}
              </div>
            </form>
